...
| Skill | 適合什麼需求 |
| --- | --- |
| `ultrathink` | 釐清尚有疑點或取捨的問題，依需要選用方法，查證假設、比較方案與反例，再提出建議。六語分析與完整謬誤檢核依需要自動啟用，也可明確指定。重大決策形成過程的偏誤檢查，會在 decision-bias-quality-control 可用時自動轉接。 |
...
| `swot-analysis` | 想做 SWOT / TOWS 分析，將內外部因素轉成候選策略並比較取捨，也可檢查或更新局部分析。 |
...
| Skill | 適合什麼需求 |
| --- | --- |
| `orchestrating-mixed-methods` | 整合質性與量化研究，支援研究設計、文獻探討、資料分析與創新應用。 |
...
| Skill | 適合什麼需求 |
| --- | --- |
| `commercial-proposal-writing` | 撰寫完整商業提案、局部改稿或純審查，涵蓋合作方案、募資 deck 與商業計畫。 |
