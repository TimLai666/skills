...
| `pestel-analysis` | 想做嚴謹的 PESTEL / 外部環境掃描，作為 SWOT 或策略規劃前置分析。 | `pestel-analysis` |
...
| `threads-viral-growth` | 想在 Threads 平台產出高互動爆文、設計社群貼文策略、趨勢研究與發文劇本。 | `threads-viral-growth` |
...
| `coding-guidelines` | 寫 code 或改 code 前先載入：想清楚再寫、最小變更、精準手術、TDD、目標驅動。 | `coding-guidelines` |
...
| `set-zeabur-conventions` | 想為專案設定 Zeabur 部署規範（寫入 AGENTS.md，給所有 agent 看）。 | `set-zeabur-conventions` |
