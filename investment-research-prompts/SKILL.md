...
取得方式依環境擇一：有網路搜尋能力就先查（標注來源與查詢日期）；沒有就請使用者提供，或明確標示「以下分析基於 [日期] 資料，時效性欄位需自行更新」後再繼續。
...
## Minimal Example (stock-screening)

使用者：「幫我篩出適合長期持有的台股標的。」

1. `detected_scenario: stock-screening`
2. Input Gate：缺「篩選範圍（市值/產業）」「持有期定義」「風險偏好」→ 回 `MissingDataOutput` 只追問這三項
3. 補齊後展開框架（節錄）：

```
角色：買方研究員，專注長期基本面選股。
篩選條件：[市值門檻]、[產業範圍]、連續 [N] 年 ROE > [x]%、
自由現金流為正、負債比 < [y]%。
輸出：候選清單（代號、名稱、通過條件、最大風險一句話）
+ 每檔的「不買理由」欄（強制填寫，避免確認偏誤）。
資料時效：標注財報季度與查詢日期。
```

其餘七個場景的完整模板見 [references/prompt-library.md](./references/prompt-library.md)。
