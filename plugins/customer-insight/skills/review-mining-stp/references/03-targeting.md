...
品質評價缺漏的資料不得補成 0 或 5。各檢定或模型依實際採用的有效資料計算，並交代有效樣本數與排除情況。提及次數與有品質評價的次數分開解讀。

客群排序只使用每群都有有效平均值的比較欄位，排除項目記入 `excluded_comparison_axes`。沒有可共同比較的欄位時，腳本以非零狀態結束並說明缺口，保留先前階段的檔案，不產生完整報告。
