...
1. 產生新 migration 檔（帶時間戳）。
2. 寫 SQL。新建表時務必同時：加 `created_at/updated_at/deleted_at`、掛 `updated_at` trigger。
3. 本機跑 migration 驗證。
4. `git add` 並 commit — migration 檔一定要進版控。
5. migration 一旦 commit 就視為不可變。要再改，寫**新的** migration。
