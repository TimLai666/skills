...
Run this before delivering. Go through the items one by one, and mark an item checked only after verifying it against the actual text, not from a general impression of the draft. Fix what fails, then run the list again until every box is checked. Each item condenses a rule from [human-writing-principles.md](./human-writing-principles.md); go back to that file when an item needs the full rule to resolve.
