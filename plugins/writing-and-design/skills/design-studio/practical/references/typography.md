...
| 檔位 | 1.25 比例下的參考值 | 用途 |
|------|--------------------|------|
| caption | 12-13px | 圖注、meta 資訊、EXIF 式小字 |
| small | 14px | 輔助說明、表格 |
...
行高不是常數，是行長的函式。行越長，眼睛回行距離越遠，需要更大的行間距當「軌道」：
...
| 源流明體/源樣明體 | 明朝體（繁向） | 思源宋改刻，保留傳統字形細節，繁體內容首選 | 安靜·古典 | GitHub ButTaiwan，OFL |
| 未來熒黑 Glow Sans | 幾何黑 | 思源黑衍生的現代幾何黑，多寬度（Compressed 可做窄長 display） | 中性-大膽·現代 | GitHub welai/glow-sans，OFL |
| MiSans / HarmonyOS Sans / OPPO Sans | 廠商 UI 黑 | 比思源黑略有性格的 UI 黑，App 原型合適 | 中性 | 各廠官網，免費商用 |

選型推理：**正文只在宋/黑/楷裡選**（其餘都是 display 字型，整段用會累）；display 想要個性時才去動得意黑/老宋/明朝體。中文字型一個頂西文十個（單檔案 5-15MB），一頁最多兩個中文字型家族，為載入和統一性兩個原因。
...
**baseline 對齊**：中西 baseline 不一致時症狀是英文單詞在中文行裡「下沉」。優先換 x-height 更高的西文體；個別 display 場景用 `vertical-align: -0.02em~-0.06em` 微調西文 span，正文別這麼修（維護成本大於收益）。
...
**中文字型載入**（單檔案 5-15MB，直接引全量會毀掉首屏）：

- 首選 Google Fonts 的 Noto SC 系（已按 unicode-range 自動切成上百個分片，瀏覽器只下用到的字）
- self-host 個性字型（霞鶩/得意黑等）必須先子集化：`cn-font-split` 或 fonttools 的 `pyftsubset`，正文字型按常用 3500 字切，display 字型按實際出現的字元切（一張海報往往只有 20 個字，子集能壓到 50KB 以內）
