...
> 從 GUIDE.md「核心哲學 #1.a」下沉的完整協議（2026-06 瘦身）。GUIDE.md 留了觸發條件 + 5 步標題 + 自檢；這裡是 5 步詳細操作、下載命令、brand-spec 模板、全流程失敗兜底、反例與代價對比。
> 觸發：任務涉及具體品牌/產品時強制執行。回 GUIDE.md 看精簡版與上下文。
...
**前置硬條件**：走協議前必須已通過「#0 事實驗證先於假設」確認品牌/產品存在且狀態已知。如果你還不確定產品是否已釋出/規格/版本，先回去搜。
...
> ⚠️ **別隻試 `curl <brand>.com/logo.svg` 就放棄**——現在的官網大多是 SPA，直連靜態路徑基本返回空殼 HTML（2026-06-06 實測 Trae 官網 5 條直連路徑全是空殼）。**數字產品 / SaaS / AI 工具優先用圖示聚合源**，命中率最高、直出乾淨 SVG。

按成功率遞減：
0. **圖示聚合源（知名數字產品/SaaS/AI 工具首選，命中率最高）**：
   ```bash
   ...
   curl -s "https://api.svgl.app?search=<brand>"   # 返回 JSON，取 route(.light/.dark) 的 svg URL 再下載
...
2. 官網 HTML 全文提取 inline SVG：
   ...
   ```
3. **Google favicon 服務（站點真實 mark 兜底，幾乎不失敗）**：
   ```bash
   curl -o logo.png "https://www.google.com/s2/favicons?domain=<brand-domain>&sz=256"   # 256px 官方站點圖示
   ```
4. 官方社交媒體 avatar（最後手段）：GitHub/Twitter/LinkedIn 的公司頭像通常是 400×400 或 800×800 透明底 PNG
...
| **產品圖（實體產品）找不到** | 優先 nano-banana-pro AI 生成（以官方參考圖為基底）→ 次選向用戶索取 → 最後才是誠實 placeholder（灰塊+文字標籤，明確標註"產品圖待補"） |
| **UI 截圖（數字產品）找不到** | 向用戶索取自己賬號的截圖 → 官方演示影片截幀。不用 mockup 生成器湊 |
| **色值完全找不到** | 按「設計方向顧問模式」走，向用戶推薦 3 個方向並標註 assumption |
...
- **五大 Coding Agent 對比 PPT（2026-06-06，觸發觸發條件擴充套件的真實案例）**：agent 把任務判成「PPT + 沒風格參考」走 Fallback 設計方向顧問，只抽了五家品牌色就 spawn 三套設計邏輯，**五個產品 logo（Claude Code / Cursor / Codex / Copilot / Trae）一個沒取**——被花叔抓現行「我們為什麼沒去取這些產品的 logo」。根因：把「對比 / 榜單 deck」誤判為不觸發 §1.a（以為 §1.a 只管「為單一客戶做物料」），且 Fallback 路徑裡沒有任何 logo 檢查點。→ 修復：①觸發條件擴成兩類（含「設計裡點名/並列真實產品」）②Fallback 不豁免取 logo ③Phase 3.5 加「具名產品 logo 子門」spawn 前必過 ④Step 3.1 補 svgl/simpleicons/Google favicon 可靠取圖鏈。
