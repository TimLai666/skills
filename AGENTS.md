...
這是一個 Agent Skills 集合 repo（60 個 skill，分成 10 個 plugin），同時是 Claude Code plugin marketplace。沒有 build、lint、test 流程，唯一的腳本是打包工具：
...
- **不自創根目錄檔案。** 用既有的協調檔（`AGENTS.md`、`CLAUDE.md`、`DESIGN.md`、`delivery-status.md`、`ENG.md`、`DESIGN-REVIEW.md`），或放進 `docs/<skill-name>/`。根目錄是使用者的，不是 skill 的工作檯。
